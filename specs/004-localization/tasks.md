...
- [ ] T001 Add `indic-transliteration` to `pyproject.toml`; create `data/i18n/ui/` and `data/i18n/content/` dirs
- [ ] T002 [P] Create `sadhana_setu/i18n.py` + `sadhana_setu/translit.py` skeletons and `tests/test_i18n.py`, `tests/test_translit.py`
...
- [ ] T007 [US1] Author `data/i18n/ui/en.yaml` (UI string keys) and replace UI literals with `i18n.t(key)` across `sadhana_setu/ui/` (`app.py`, `prejapa_view.py`, `nama_tattva_view.py`, `notes_view.py`, `saturday_view.py`, `today_view.py`, `this_week_view.py`, `history_view.py`)
- [ ] T008 [US1] Add a language selector (English / తెలుగు / ಕನ್ನಡ / தமிழ்) to the sidebar in `sadhana_setu/ui/app.py` → `i18n.set_locale`
- [ ] T009 [US1] Draft the Telugu UI catalog `data/i18n/ui/te.yaml` via `scripts/draft_translations.py` (`reviewed` pending native review)
- [ ] T010 [P] [US1] Test UI fallback (missing `te` key ⇒ English) in `tests/test_i18n.py` (SC-001)
...
- [ ] T015 [US3] Wire `translit.to_script` into verse/Sanskrit-term rendering for vernacular locales (faith-verse IAST, nāma-tattva terms) in the relevant views/`i18n.py` helper
- [ ] T016 [P] [US3] Test that verse/term rendering in a vernacular locale is transliterated (and IAST-fallback on miss) in `tests/test_translit.py`
