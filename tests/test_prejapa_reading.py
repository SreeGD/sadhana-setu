...
def _no_corpus(name, args):
    return []  # search_corpus returns nothing ⇒ curated fallback


def _corpus_hit(name, args):
    return [{"text": "Hear one Hare Kṛṣṇa mantra attentively.",
             "metadata": {"kind": "harinaam-note", "speaker": "Bhūrijana Prabhu",
                          "title": "Holy Name 01"}}]
...
def test_all_four_stages_present():
    r = build_reading(FIXED, caller=_no_corpus, checkin_loader=lambda d: None)
...
def test_corpus_teaching_used_when_available():
    r = build_reading(FIXED, caller=_corpus_hit, checkin_loader=lambda d: None)
...
def test_sankalpa_echo_present_and_absent():
    with_echo = build_reading(FIXED, caller=_no_corpus, checkin_loader=lambda d: _Checkin())
    assert with_echo.sankalpa_echo and "early rising" in with_echo.sankalpa_echo
    without = build_reading(FIXED, caller=_no_corpus, checkin_loader=lambda d: None)
    assert without.sankalpa_echo is None


def test_daily_stable():
    a = build_reading(FIXED, caller=_no_corpus, checkin_loader=lambda d: None)
    b = build_reading(FIXED, caller=_no_corpus, checkin_loader=lambda d: None)
